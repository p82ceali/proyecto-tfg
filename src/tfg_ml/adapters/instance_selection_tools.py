...
Tools (sampling):
    • StratifiedSampleTool
        Class-stratified subsampling by proportion.
    • RandomSampleTool
        Uniform random subsampling by proportion.
    • ClassBalancedSampleTool
        Per-class quota (or global cap) sampling to balance class counts.
    • ClusteredSampleTool
        Diversity sampling using KMeans centroids (nearest points per cluster).

Tools (splits):
    • TrainValTestSplitTool
        Reproducible train/val/test split; optional stratification by target.
    • TimeSeriesSplitTool
...
Usage requirements:
    ...
    - Side effects:
        ...
        Splitting tools persist to `data/train.csv`, `data/val.csv`, `data/test.csv`.

Traceability:
    Each tool records a compact decision entry via `CTX.add_decision(...)`.
"""
...
def _numeric_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Raises:
        ValueError: If there are no numeric columns.
    """
...
class StratifiedSampleInput(BaseModel):
    """Inputs for StratifiedSampleTool."""
...
class StratifiedSampleTool(BaseTool):
    """
    Subsample using stratified sampling on the given target column.

    Notes:
        - Requires each class to have at least 2 samples for stratification.
        - Persists the reduced dataset to `data/dataset.csv`.
    """
    ...
    def _run(self, target: str, sample_size: float = 0.5, random_state: Optional[int] = 42) -> str:
        ...
        if any(cnt < 2 for cnt in class_counts.values()):
            return (
                "Cannot apply stratified sampling: a class has < 2 samples. "
...
class RandomSampleTool(BaseTool):
    """
    Uniform random (non-stratified) subsampling by proportion.
...
class ClassBalancedSampleInput(BaseModel):
    """Inputs for ClassBalancedSampleTool."""
...
class ClassBalancedSampleTool(BaseTool):
    """
    Construct a class-balanced subset using per-class quotas or a global cap.
...
class ClusteredSampleInput(BaseModel):
    """Inputs for ClusteredSampleTool."""
...
class ClusteredSampleTool(BaseTool):
    """
    Diversity-based sampling via KMeans on numeric features.

    For each cluster, selects the nearest point(s) to the centroid as representatives.
    Side effects:
        Persists the reduced dataset to `data/dataset.csv`.
    """
    ...
    def _run(
        ...
        for cl in range(n_clusters):
            ...
            # Primary representative: nearest to centroid
            ...
            # Additional representatives by ascending distance
...
class TrainValTestSplitInput(BaseModel):
    """Inputs for TrainValTestSplitTool."""
...
class TrainValTestSplitTool(BaseTool):
    ...
    Behavior:
        - If `target` is provided and `stratify=True`, uses stratified splits.
        - Validation set is drawn from the remaining portion after test split.

    Side effects:
        Persists splits to `data/train.csv`, `data/val.csv`, and `data/test.csv`.
    """
    ...
    def _run(
        ...
        # Validation is carved from the remaining (train+val) portion
        ...
        os.makedirs("data", exist_ok=True)
        train_path, val_path, test_path = "data/train.csv", "data/val.csv", "data/test.csv"
...
class TimeSeriesSplitInput(BaseModel):
    """Inputs for TimeSeriesSplitTool."""
...
class TimeSeriesSplitTool(BaseTool):
    """
    Chronological (no-leakage) train/val/test split by a time column.
